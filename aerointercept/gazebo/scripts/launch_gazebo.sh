...
case "$MODE" in
  ...
  mixed)
    ...
    echo "[AeroIntercept] mixed will switch seeded scenarios at episode reset"
    ;;
  hover|figure_eight|stop_go) ;;
  *) echo "unsupported target scenario: $MODE" >&2; exit 2 ;;
esac

for required in "$WORLD_FILE" "$BRIDGE_CONFIG" "$PX4_ROOT/build/px4_sitl_default/bin/px4"; do
  if [[ ! -e "$required" ]]; then
...
/usr/bin/python3 -s -m aerointercept.gazebo.prepare_models \
  --source "$PX4_ROOT/Tools/simulation/gz/models" --output "$RUNTIME_DIR/models"
/usr/bin/python3 -s -m aerointercept.gazebo.scripts.audit_model_frames \
  --models "$RUNTIME_DIR/models" --output "$RUNTIME_DIR/model_frame_audit.json"
PARK_MODEL_ROOT="$PROJECT_ROOT/assets/gazebo/models/aerointercept_park_surface"
export GZ_SIM_RESOURCE_PATH="$RUNTIME_DIR/models:$PROJECT_ROOT/assets/gazebo/models:$PARK_MODEL_ROOT:$PARK_MODEL_ROOT/materials/textures:$PX4_ROOT/Tools/simulation/gz/models:$PX4_ROOT/Tools/simulation/gz/worlds${GZ_SIM_RESOURCE_PATH:+:$GZ_SIM_RESOURCE_PATH}"

echo "[AeroIntercept] logs: $RUNTIME_DIR"
if ((HEADLESS)); then
  gz sim -v 4 -r -s "$WORLD_FILE" >"$RUNTIME_DIR/gazebo.log" 2>&1 &
else
...
(
  ...
  exec env PX4_GZ_STANDALONE=1 PX4_GZ_WORLD=aerointercept_park \
    PX4_GZ_MODELS="$RUNTIME_DIR/models" \
...
(
  ...
  exec env PX4_GZ_STANDALONE=1 PX4_GZ_WORLD=aerointercept_park \
    PX4_GZ_MODELS="$RUNTIME_DIR/models" \
    PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE="0,$TARGET_SPAWN_NORTH_M,0" PX4_SIM_MODEL=gz_x500 \
...
/usr/bin/python3 -s "$PROJECT_ROOT/aerointercept/gazebo/ros_bridge.py" \
  --socket "$SOCKET_PATH" --image-topic /camera/image \
  --target-origin-ned "$TARGET_SPAWN_NORTH_M" 0 0 --reset-position-ned 0 0 -6 \
  --camera-mount-yaw-offset 0 --mode "$MODE" --seed "$SEED" \
...
echo "[AeroIntercept] cooperative target supervisor mode: $MODE seed: $SEED"
